...
<form name="icform" method="post">
	<input type="text" name="contract.id" value="${contract.id}"/>
...
<div>
	...
	<table id="ec_table"  border="0"  cellspacing="0"  cellpadding="0"  class="tableRegion"  width="98%" >
		...
		<tr class="odd"  onmouseover="this.className='highlight'"  onmouseout="this.className='odd'" >
			...
			<td>${boxNum}</td>
			<td>
				<a href="contractProductAction_toupdate?contract.id=${contract.id}&id=${id}">[修改]</a>
				<a href="contractProductAction_delete?contract.id=${contract.id}&id=${id}">[删除]</a>
				<a href="extCproductAction_tocreate?contractProduct.id=${id}">[附件]</a>
			</td>
		</tr>
		</s:iterator>
